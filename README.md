...
![进度](https://img.shields.io/badge/已标注-41%20%2F%20100%20集-yellow)
...
> 规模化质检 = **只看 `flags` 非空的那几条**。当前 41 集 **0 个 flag**。
...
在已下载的 **41 条** episode 上(每条 1049–1266 帧,均值约 1168;总时长约 38.9s):**0 个 flag**,各子任务时长跨集高度一致。

| 子任务 | 均值 (s) | 范围 (s) |
|------|------|------|
| S0 伸手 + 抓取 | 6.4 | 5.5 – 7.4 |
| S1 倒入 | 6.3 | 4.6 – 9.0 |
| S2 放下容器 + 取杵 | 11.8 | 10.0 – 14.0 |
| S3 研磨 | 9.6 | 7.2 – 12.5 |
| S4 抬杵 + 收回 | 4.8 | 3.2 – 7.8 |

> 数据集共 100 条,目前下载了 41 条;其余下载完后重跑同一条命令即可覆盖全量。
...
- 最可靠的是 `b1 / b2`(抓取 / 松手);**最软的是 S2↔S3**(把杵搬到位 → 开始磨是渐变的,约 ±0.5s)。
- 相机是低照度鱼眼,所以标注主要靠本体感知信号、不靠像素。
- `ENGAGE_DIM = 3` 是针对**本数据集的本体**验证出来的(在 41 集上稳定);换数据集 / 换机器人需要重新确认这个维度。
